...
if staging.is_dir():
    ...
    for version in ['v2']:
        ...
        _tracked_paths.add(library)
        s.copy([library], destination='src/admin', excludes=['package.json', 'README.md', 'src/index.ts', 'src/v2/index.ts', 'tsconfig.json', 'tslint.json', 'system-test/fixtures/sample/src/index.ts', 'system-test/fixtures/sample/src/index.js', '.github/sync-repo-settings.yaml', 'protos', 'system-test', '.eslintignore', '.eslintrc.json', '.gitignore', '.jsdoc.js', '.mocharc.js', '.prettierrc.js','webpack.config.js'])
