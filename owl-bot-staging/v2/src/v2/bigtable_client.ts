 ...
 */
  ...
  mutateRow(
    ...
    options.otherArgs.headers = options.otherArgs.headers || {};let routingParameter = {};
      if(RegExp('(?<table_name>projects)/[^/]+/instances/[^/]+/tables/[^/]+').test(request.table_name!)){  
        Object.assign(routingParameter, { table_name: request.table_name!.match(RegExp('(?<table_name>projects/[^/]+/instances/[^/]+/tables/[^/]+)'))![0]})
      } else
      {}
      if(RegExp('[^/]+').test(request.app_profile_id!)){  
        Object.assign(routingParameter, { app_profile_id: request.app_profile_id!.match(RegExp('[^/]+'))![0]})
      } else
      {}  
